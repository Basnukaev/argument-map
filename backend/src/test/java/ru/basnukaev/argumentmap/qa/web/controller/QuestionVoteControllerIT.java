...
/**
 * IT для {@link QuestionVoteController} - REST голосования за вопросы Q&amp;A.
 * Зеркалит {@code TopicVoteControllerIT} но без visibility/403 - questions это
 * open discussion (любой authenticated user голосует) + проверяет что
 * question list/detail отдают voteScore.
 */
@SpringBootTest
@AutoConfigureMockMvc
@Import(TestcontainersConfiguration.class)
@Transactional
class QuestionVoteControllerIT {
    ...
    void POST_vote_upvote_returns201_andStats() throws Exception {
        var req = new CreateQuestionVoteRequest(1);

        mockMvc.perform(post("/api/v1/questions/{id}/vote", questionId)
    ...
    void POST_vote_changeVote_upserts() throws Exception {
        mockMvc.perform(post("/api/v1/questions/{id}/vote", questionId)
                        .header("X-User-Id", userId.toString())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new CreateQuestionVoteRequest(1))))
                .andExpect(status().isCreated());
    ...
    void POST_vote_invalidWeight_returns400() throws Exception {
        var req = new CreateQuestionVoteRequest(2);
    ...
    @Test
    void POST_vote_questionMissing_returns404() throws Exception {
        var req = new CreateQuestionVoteRequest(1);

        mockMvc.perform(post("/api/v1/questions/{id}/vote", UUID.randomUUID())
                        .header("X-User-Id", userId.toString())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.type").value(containsString("question-not-found")));
    }
    ...
    void DELETE_vote_existing_returns204() throws Exception {
        ...
        mockMvc.perform(delete("/api/v1/questions/{id}/vote", questionId)
                        .header("X-User-Id", userId.toString()))
                .andExpect(status().isNoContent());

        // голоса больше нет - GET stats показывает чистое состояние
        mockMvc.perform(get("/api/v1/questions/{id}/votes", questionId)
                        .header("X-User-Id", userId.toString()))
                .andExpect(jsonPath("$.upvotes").value(0))
    ...
    @Test
    void GET_votes_questionMissing_returns404() throws Exception {
        mockMvc.perform(get("/api/v1/questions/{id}/votes", UUID.randomUUID())
                        .header("X-User-Id", userId.toString()))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.type").value(containsString("question-not-found")));
    }
    ...
    @Test
    void GET_questionDetail_includesVoteScore() throws Exception {
        mockMvc.perform(post("/api/v1/questions/{id}/vote", questionId)
                        .header("X-User-Id", userId.toString())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new CreateQuestionVoteRequest(1))))
    ...
    @Test
    void GET_questionList_includesVoteScore() throws Exception {
        mockMvc.perform(post("/api/v1/questions/{id}/vote", questionId)
                        .header("X-User-Id", userId.toString())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new CreateQuestionVoteRequest(1))))
        ...
        mockMvc.perform(get("/api/v1/questions")
                        .header("X-User-Id", userId.toString()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.items[?(@.id=='" + questionId + "')].voteScore").value(2))
                .andExpect(jsonPath("$.items[?(@.id=='" + questionId + "')].userVote").value(1));
    }

    @Test
    void GET_questionList_noVotes_defaultsToZero() throws Exception {
        mockMvc.perform(get("/api/v1/questions")
                        .header("X-User-Id", userId.toString()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.items[?(@.id=='" + questionId + "')].voteScore").value(0));
    }

    @Test
    void GET_questionDetail_noVotes_userVoteNull() throws Exception {
        mockMvc.perform(get("/api/v1/questions/{id}", questionId)
                        .header("X-User-Id", userId.toString()))
                .andExpect(status().isOk())
