...
или эксперте, мнение которого цитируется. Поле `type` (миграция 47):
`SCHOLAR` (учёный-муфассир / муҳаддис), `AUTHOR` (автор книги),
`MUHAQQIQ` (тахкик), `PUBLISHER` (издательство), `OTHER`. Default
`SCHOLAR`. До миграции 47 был flat namespace - один UUID мог обозначать
любую роль. Семантическая валидация: `HadithGradeService.addGrade`
принимает только authorities с `type=SCHOLAR` (оценивать хадис может
только муҳаддис, не издательство).
...
Пророка ﷺ к составителю сборника (Бухари, Муслим и т.д.).
Визуализируется в Hadith Explorer как граф (ADR-049): узлы-передатчики
+ синтетический корень Пророка ﷺ, рёбра с формулами передачи. См.
`src/apps/hadith/components/SanadGraph.tsx`.
...
**Matn (матн)** — текст хадиса (что сказано), в отличие от иснада
(кем передано). Один хадис может иметь несколько matn-вариаций в
разных сборниках с минимальными различиями в формулировке.

**Tahammul (формула передачи)** — глагол передачи на ребре иснада:
`سمعت` (слышал лично), `حدثنا` (рассказал нам), `أخبرنا` (сообщил нам),
`عن` («от» — анана, менее явная). Явное слушание (самаа) сильнее анана.

**Thiqa / надёжность передатчика** — степень доверия в науке рижаль:
`THIQA` (надёжный) > `SADUQ` (правдивый) > `MAQBUL` (приемлемый) >
`DAIF` (слабый) > `MATRUK` (оставленный). `SAHABI` (сподвижник) — вне
шкалы джарх ва тадиль (по иджме 'удул). `UNKNOWN` (маджхуль). Whitelist
в `hd_narrators.reliability_grade` (миграция 56 добавила `SAHABI`).

**Common link / мадар** — передатчик, в котором сходятся, а затем
расходятся несколько цепей одного хадиса. Для «Дела по намерениям» —
Яхья ибн Саид аль-Ансари: иснад «гариб» (одиночный) выше него и
«машхур» (расходится на многих) ниже. На графе sanad — точка fan-out.

**Табака (طبقة, tabaqa)** — поколение/слой передатчиков (сподвижники,
табиины, табиут-табиин и т.д.). В alminasa приходит как свободный
арабский ordinal в поле `level` (`الثانية`, `العاشرة`, `صحابي`, …);
хранится дословно в `hd_narrators.tabaqa`.

**Такхридж (تخريج, takhrij)** — установление всех мест и путей, где
встречается хадис (в каких сводах, под какими номерами, какими
цепочками). В alminasa — массив параллельных преданий `raw_narrations`;
у нас раскладывается в `hd_hadith_crossrefs`, граф «все طرق» строится
агрегацией cross-refs.

**Турук (طرق, turuq)** — множественные пути/цепочки одного и того же
предания (мн.ч. от طريق, тарик). Под моделью «один хадис = одна цепочка»
(ADR-060) разные турук — это отдельные `hd_hadiths`, связанные через
`hd_hadith_crossrefs`.

**Иляль (علل, ʿilal)** — скрытые дефекты иснада или матна, незаметные
при поверхностном взгляде (например, скрытый обрыв цепи или подмена
передатчика). Наука علل الحديث — высший уровень критики. В alminasa —
вкладка علل; хранится в `hd_explanations` с `kind=ILAL`.

**Гариб (غريب الحديث, gharīb al-hadith)** — объяснение редких,
непонятных или устаревших слов в матне хадиса. Не путать с «гариб» как
характеристикой одиночной цепи (см. common link). В alminasa — вкладка
غريب; хранится в `hd_explanations` с `kind=GHARIB`.

**Джарх ва та'диль (الجرح والتعديل, al-jarḥ wa-t-taʿdīl)** — наука
критики (جرح) и подтверждения надёжности (تعديل) передатчиков хадисов.
Оценка конкретного рави формулируется дословно (`ثقة ثبت`, `صدوق يهم`
и т.п.). В alminasa приходит в поле `grade`; хранится verbatim в
`hd_narrators.grade_text`, грубая производная — в enum
`reliability_grade` (THIQA/SADUQ/…) для фильтров и цвета.
...
Содержит `extracted_text` из ручного ввода или будущего AI-recognition.
Используется для точного цитирования рукописей.
...
тахкиков и для AI editing pass (ADR-042, Этап 17.e).
