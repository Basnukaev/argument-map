...
```jsonc
{
  ...
  "explanations": [ { "kind": "SHARH", "bookName": "فتح الباري",
    "author": "ابن حجر", "authorDeathYear": 852, "page": 3, "volume": 1,
    "text": "شرح...", "reference": null },
    { "kind": "GHARIB", "bookName": "النهاية في غريب الحديث",
    "author": "ابن الأثير", "authorDeathYear": 606, "page": 261, "volume": 1,
...
### Hide/reveal (Фаза 4, §4.3)

Скрытие записи целиком — `PUT` с `fieldName='__record__'`, `hidden=true`,
обязательным `reason` (разрешено для hd_rulings/hd_explanations/
hd_narrator_commentaries/hd_matns/hd_sanads; для hd_hadiths/hd_narrators —
400). На чтении detail-эндпоинтов (`GET /hadiths/{id}/detail`,
`GET /narrators/{id}`):
- **обычный читатель/гость** — скрытая запись **вырезана** из списка вовсе;
- **ADMIN** (по роли запрашивающего) — запись приходит с `hiddenByAdmin=true`
  + `hideReason`, чтобы раскрыть (DELETE override).

DTO `RulingDto`/`ExplanationDto`/`NarratorCommentaryDto` получили `id` (для
адресации hide фронтом) + `hiddenByAdmin`/`hideReason` (reveal-режим).

### Field-edit сателлитов на detail (Фаза 5, §5)

На тех же detail-эндпоинтах field-level правки overlay теперь накладываются
и на сателлиты (не только на сам хадис/рави): `hd_rulings`, `hd_explanations`,
`hd_narrator_commentaries`, `hd_matns` (мета: `printed_number`/`page_no`/
`volume`/`divergence_summary`/`text_ru`/`text_en`), `hd_sanads` (`chain_grade`/
`primary_chain`). EFFECTIVE-значение поля видно в ответе; поле-уровневое
скрытие (`hidden=true` на колонке) отдаёт поле как `null` (`ruler_name`,
`explanation.author`/`text`, `commenter`). Первоисточник по-прежнему вне
whitelist (`hd_matns.text_ar`/`text_ar_normalized`, commentary `comments` →
400 `curation-field-not-editable`). Один батч-load overrides на тип
(`OverrideApplyService.applyAndHide` — field-override + record-hide в один
