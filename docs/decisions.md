...
## ADR-060: alminasa.ai — единственный источник хадисов
**Дата:** 2026-06-03
**Статус:** принято (заменяет решение Сессии 55 «sunnah primary + AI-иснад»
и ADR-059 в части AI-извлечения иснада)
**Реализовано:** Сессия 56 — фундамент схемы (миграции 70+71, доменные
records, репозитории); краулер и маппер — отдельными планами (см. спеку §F).

**Контекст.** Сессия 55 зафиксировала sunnah.com как primary-источник
матна + AI-извлечение иснада из free-text матна (ADR-059). HAR-анализ
обоих кандидатов (sunnah.com vs alminasa.ai, HAR-файлы в корне репо)
показал принципиальную разницу: sunnah.com отдаёт только
матн + переводы + базовый grade, тогда как **alminasa.ai отдаёт
структурно всё нужное для хадисоведения** — пред-связанный иснад с
устойчивыми numeric-ID рави, риджаль-биографии с джарх-та'диль (`grade`),
тип хадиса (`type`: марфу'/маукуф/…), такхридж/طرق (`raw_narrations`),
рулинги (вердикты учёных с годом смерти) и шарх (`hadith-explanation-12`).
Объём: ~82,596 хадисов / 12 сборников / 11,221 رواة. Иснад приходит как
упорядоченный массив `narrators[]` + inline-разметка рави в тексте
(`<a class=rawy id=N>…</a>`) — то есть **детерминированный парс вместо
AI-извлечения** (которое галлюцинирует и не даёт стабильных ID рави).

**Решение.** alminasa.ai = **единственный источник** арабского контента и
хадисоведческих данных.

- **Доступ — bulk-снапшот через staging в нашу БД.** alminasa выставляет
  открытый read-only Elasticsearch reactivesearch-прокси
  (`POST /api/reactivesearchproxy/{index}-read/_search` либо `_msearch`,
  тело — ES Query DSL; без авторизации, проверяет лишь `Origin`/`Referer`).
  Индексы `hadith-12`, `narrators-12`, `hadith-explanation-12`,
  `rulings-12_v2` (+ вероятно ilal/gharib) краулятся в `am_staging_*`
  (сырой JSONB + разобранные горячие поля), оттуда маппятся в домен `hd_*`.
  **Рантайм НИКОГДА не ходит в alminasa** — снапшот наш, с атрибуцией.
- **Модель — 1:1 атомарно + cross-refs.** Каждый alminasa `hadith_id`
  (формат `{bookId}-{serial}`, напр. `146-…` = صحيح البخاري) → один
  `hd_hadith` (один матн, одна печатная цепочка). Связи «то же предание
  другим путём» (طرق/такхридж) — через `hd_hadith_crossrefs`; граф «все طرق»
  строится агрегацией cross-refs на чтении. Идемпотентность импорта — по
  природным ключам `external_id` (хадис) и numeric narrator id (рави).
- **Переводы ru/en — AI on-demand** (swappable `LlmClient`, ADR-058), а не
  частью снапшота.

**Причины.** Авторитетные пред-связанные цепочки + риджаль с устойчивыми
ID детерминированным парсом устраняют главный риск ADR-059 —
AI-галлюцинации в иснаде и наивный fuzzy-матчинг имён рави. Один источник
вместо двух убирает дублирующий ETL и рассинхрон. Снапшот в свою БД снимает
зависимость рантайма от датированного (`2024-08-24`) и юридически серого
внешнего прокси.

**Последствия.**
- (a) Решение Сессии 55 (sunnah primary + AI-иснад) и **ADR-059 в части
  AI-извлечения иснада — superseded**: alminasa даёт цепочки
  детерминированным парсом. **ADR-058 (swappable LLM) остаётся в силе** —
  AI перенацеливается на перевод (ar→ru/en), Q&A, гариб, PDF-парсинг.
  Визуализация `SanadGraphService`/`SanadGraphResponse` + фронтовый
  `SanadGraph` остаётся (переиспользуется на данных alminasa).
- (b) sunnah ETL (`hadith/sunnah/**`, `sn_staging_*`, `AdminSunnahPage`,
  dev-конфиг `sunnah.dump.*`, docker `sunnah-mysql`) и AI-isnad
  (`hadith/isnad/**`, endpoint `extract-isnad`, фронтовая кнопка) — **код
  удаляется отдельным планом**.
- (c) Схема расширена **миграциями 70+71**: на `hd_hadiths` —
  `external_source`, `external_id` (+ UNIQUE с source), `hadith_type`,
  `chapter_ar`, `sub_chapter_ar`, `full_text_ar`; на `hd_narrators` —
  `external_source`, `external_id` (+ UNIQUE), `tabaqa`, `grade_text`,
  `born_on_text`, `died_on_text`; новые таблицы `hd_hadith_editions`,
  `hd_rulings`, `hd_explanations` (`kind` SHARH/ILAL/GHARIB),
  `hd_hadith_crossrefs`, `hd_narrator_relations`. Enum
  `reliability_grade`/`hadith_grades` остаются для грубых производных и
  ручных пользовательских оценок (ортогонально импортированным рулингам).
- (d) Юридически: открытый ES-прокси — серая зона при bulk-краулинге; риск
  снижаем снапшотом в свою БД с атрибуцией источника в метаданных хадиса,
...
## ADR-063: Двух-осевая таксономия хадиса — провенанс + аутентичность (выводимая)
**Дата:** 2026-06-17
**Статус:** принято
**Реализовано:** Сессия 62, миграция 77, спека
`docs/specs/2026-06-17-hadith-explorer-ux-feedback.md` (C8/C19/C21/D1)

**Контекст.** Поле `hd_hadiths.status` (CANONICAL 5488 / VARIANT 26511) — это
**провенанс** (CANONICAL = из Сахихайн, VARIANT = параллельная передача из
других сборников). Но UI-фильтр смешивал его с **аутентичностью**
(Слабые/Выдуманные), которой в `status` нет вообще → daif/maudu-фильтры всегда 0.
Две ортогональные оси хадисоведения слиты в одну (претензия Абдулы по ручному
тесту С62).

**Решение.** Разделить на 2 оси:
1. `status` = провенанс (без изменений; UI переименовывает CANONICAL/VARIANT
   через i18n+tooltip, не дословно «Канонический/Вариант»).
2. Новая `hd_hadiths.authenticity` (SAHIH/HASAN/DAIF/MAUDU; миграция 77,
   nullable + CHECK-whitelist) = достоверность, **выводимая** из рулингов
   (`hd_rulings.ruling_text`) keyword-эвристикой в `AlminasaHadithMapper.deriveAuthenticity`.
   UI: две группы фасетов «Происхождение» + «Достоверность» (ортогонально, AND).
   `hadith_type` (مرفوع/موقوف/مقطوع/قدسي — настоящие хадисоведческие термины) —
   i18n+tooltip (марфу'/маукуф/макту'/кудси).

**Эвристика (приближённая).** Keyword-матч в нормализованном пространстве
(`ArabicTextNormalizer`): موضوع→MAUDU, ضعيف→DAIF, حسن→HASAN, صحيح→SAHIH.
Per-ruling — худший бакет (MAUDU>DAIF>HASAN>SAHIH; «حسن صحيح»→HASAN); затем
модальный (самый частотный) по корпусу рулингов хадиса, худший при равенстве;
null если нет совпадений. **Ограничения** (в javadoc `deriveAuthenticity` + типе
`HadithAuthenticity`): подстрочный матч путает отрицания («ليس بموضوع»→MAUDU) и
контекст. Для фасетного фильтра достаточно; ручную оценку учёного
(`hadith_grades`, ADR-062) НЕ заменяет.

**Последствия.** `authenticity` заполняется на re-map (`import/hadiths`); до
...
**Статус:** ✅ Принято (Сессия 65, 2026-06-24); **трек закрыт — Фазы 0-6
реализованы** (Фаза 6: миграция C9-перевода в overlay, P0-1a-страховка снята).
...
строку с `text_ru/en = NULL` и новым `matnId` (промежуточно спасался merge-
страховкой P0-1a в `AlminasaHadithMapper`/`MatnRepository.findPrimaryByHadithId`
— **снята в Фазе 6**, перевод уехал в overlay под стабильным ключом).
...
- **(C) Merge-стратегия (upsert сателлитов по природному ключу, сохраняя
  ...
  перевода** (P0-1a) до готовности overlay; снята в Фазе 6 (перевод в overlay).
...
- **Фаза 6 (закрыта, трек завершён):** ключ matn-перевода был нестабилен на
  реимпорте (`matn.id` = новый UUID) → решено (§10 вопрос 2, авторизовано
  Абдулой) ключевать перевод primary-матна по СТАБИЛЬНОМУ
  `(entity_table='hd_matns', entity_id=hadith_id, field_name='primary_text_ru'/
  'primary_text_en')` — синтетические ключи (не реальные колонки), резолв и
  наложение на primary-матн на apply (`OverrideApplyService.applyMatns`).
  Не-primary матн — per-variation `(entity_id=matn.id, field_name='text_ru'/
  'text_en')` (на реимпорте не переживёт, но это не накопленный C9-кейс).
  C9-эндпоинт `PATCH /matns/{id}/translation` переписан на overlay внутри
  (URL сохранён, фронт не трогаем); AI `/translate` пишет в базовую колонку,
  human-правка накладывается поверх. Data-миграция `20260618-79` перенесла
  накопленный перевод (`edited_by`=первый ADMIN). **P0-1a-страховка снята**
...
**Правка ребра — только в одноцепочечном (main) виде** (решение по ревью 5.b-B):
формула передачи — свойство конкретного звена `(hadith_id, position)`. В turuq-
графе («Все пути») цепи разных хадисов объединены, и ребро sibling-цепи несёт
position чужого хадиса; фронт стемпил бы его hadith_id страницы → правка ушла бы
НЕ В ТОТ хадис (одинаковые малые position коллизируют, бэк нашёл бы «какое-то»
звено с тем position и молча записал чужой override). Плюс в merged-виде
неоднозначно «формулу какой цепи правлю». Поэтому `SanadGraph.edgesEditable`
гейтит правку ребра на `viewMode==='main'`; в turuq чип read-only (индикатор
`overridden` всё равно показывается). Правка РАВИ (по стабильному `narratorId`)
работает в обоих видах — там неоднозначности нет. Альтернатива «нести per-edge
hadith_id с бэка» отвергнута (turuq-правка формул семантически мутная +
корпус одноцепочечный, YAGNI).
