...
>   `OperationCustomizer`
>
> Resolved-записи **без** active rule (просто history) архивируются в
> `docs/archive/gotchas-resolved.md` (Tashkeel-transform — фича выпилена;
> транзиентные заметки сессий).
...
## setRollbackOnly не наблюдаем из @Transactional-теста

**Симптом:** IT-тест с `@Transactional` на классе проверяет dry-run метод
(`mapHadith` + `TransactionAspectSupport...setRollbackOnly()`), assert
«строк в БД нет» падает — строки видны.

**Причина:** тест и сервис делят ОДНУ транзакцию (Spring test-managed tx
propagation REQUIRED). `setRollbackOnly` помечает общую транзакцию, но
внутри неё изменения остаются видимыми; откат случится лишь при завершении
теста — наблюдать его изнутри невозможно.

**Решение:** dry-run rollback-поведение тестировать из НЕтранзакционного
IT-класса (`AlminasaDryRunIT`): сервис открывает СВОЮ транзакцию, rollback
реально происходит, тест видит чистую БД. Очистка staging — `@AfterEach`
вручную. (План 3, Сессия 57.)

---

## Арабские литералы с диакритикой: combining-знаки не NFC-каноничны

**Симптом:** `assertThat(actual).isEqualTo("الْحُمَيْدِيُّ …")` падает,
хотя строки В КОНСОЛИ выглядят идентично (expected/actual неотличимы).

**Причина:** порядок combining-знаков (шадда U+0651 / дамма U+064F) в
данных alminasa НЕ NFC-канонический; литерал, набранный в редакторе,
получает другой порядок байт. Визуально рендер одинаков.

**Решение:** сравнивать арабские строки с диакритикой в нормализованном
пространстве (`ArabicTextNormalizer.normalize()` обе стороны) либо
ожидаемое значение брать программно из той же фикстуры. Не хардкодить
диакритизированные литералы в asserts. (План 3, Сессия 57.)

---

## alminasa-формулы и level: писцовые сокращения и вариативные формы

**Симптом:** у половины звеньев Мустадрака `transmission_phrase = NULL`;
Абу Хурайра отрендерен с бейджем «маджхуль» (UNKNOWN).

**Причина:** (1) в живых текстах формулы передачи сплошь писцовыми
аббревиатурами — `ثنا` (=حدثنا), `ثني` (=حدثني), `أنا` (=أخبرنا), `نا`,
`أنبأ` — которых не было в приоритетном списке парсера; (2) `level`
сподвижника бывает не только `صحابي`, но и `الصحابي الجليل`/`صحابية` —
строгое равенство пропускало.

**Решение:** аббревиатуры в словаре парсера с канонизацией к полным
формам (стрелки графа единообразны); SAHABI-детекция — contains корня
`صحاب` в level + startsWith в gradeText (но НЕ contains в gradeText —
«روى عن الصحابة» у табиина не должен ловиться). Данные лечатся
идемпотентным ре-маппингом. Урок тот же, что и с serial-курсором С56:
**контракт живых данных шире фикстур — правила матчинга внешнего
контента проверять на полном корпусе.** (Сессия 58.)

---

## Refresh-ротация: гонка bootstrap vs 401-interceptor → флаки-логаут по F5

**Симптом:** при перезагрузке страницы залогиненного юзера иногда
выкидывает на /login; в консоли `401 POST /api/v1/auth/refresh` (до
трёх refresh-запросов на одну перезагрузку). Воспроизводится флаки —
зависит от порядка ответов.

**Причина:** single-flight для refresh жил только в interceptor'е
`client.ts`, а bootstrap (`loadCurrentUser`) звал
`authStore.refreshAccessToken()` напрямую, минуя его. На reload оба
пути (плюс второй батч interceptor'а после microtask-release) слали
конкурентные `POST /auth/refresh` с ОДНОЙ кукой. Ротация (ADR-047)
валидирует только первый — проигравшие получают 401, и их catch стирал
сессию, затирая только что установленную победителем.

**Решение:** single-flight перенесён НА УРОВЕНЬ СТОРА (модульный
`refreshInFlight` в `authStore.ts`) — bootstrap и interceptor шарят
один in-flight promise. Дедуп только конкурентных вызовов:
...
**Канонический URL — только `localhost:5173`.** URL по WSL-IP
(`http://172.27.x.x:5173`) годится лишь как аварийный доступ: refresh-кука
имеет атрибут `Secure`, а Chrome пускает Secure-куки по plain-http ТОЛЬКО
для localhost — на IP-origin кука молча отбрасывается, логин работает,
но любой F5 разлогинивает (нет refresh). Второй Network-адрес vite
(`10.255.255.254`) — внутренний DNS-интерфейс WSL, из Windows недоступен
по дизайну, его крутилка — норма. Смежная ловушка: клик «Войти» в окне
рестарта бэка (am-backend) даёт транзиентный 403, который LoginPage мапит
в «Аккаунт отключён» — выглядит страшно, лечится повтором через 20 сек.
Если localhost-relay снова отвалится целиком — `wsl --shutdown` из
PowerShell (УБИВАЕТ всю WSL-сессию вместе с Claude и серверами — делать
осознанно).

---

## StrictMode + unmounted-ref гард = вечный спиннер (результат фетча молча выброшен)

**Симптом:** клик «Все пути» — «Строим граф иснада» крутится вечно,
хотя curl того же endpoint'а отвечает мгновенно и полно. То же у
«Параллельных текстов».

**Причина:** review-минор С59 добавил в хендлер-фетчи гард
`const unmounted = useRef(false)` + `useEffect(() => () => {
unmounted.current = true; }, [])`. В dev приложение под `<StrictMode>`:
React симулирует remount (setup → cleanup → setup), cleanup ставит флаг
в true, а setup его НЕ сбрасывает — компонент навсегда «размонтирован».
Все последующие `.then/.finally` молча скипают setState → спиннер
вечный. Классический isMounted-антипаттерн: защищал от несуществующей
проблемы (React 18+ сам безопасно глушит setState после unmount,
warning удалён), а реальный P1 создал.

**Решение:** гарды ВЫПИЛЕНЫ (HadithDetailPage, SiblingMatns) — setState
в then/catch/finally без условий. Если когда-то реально понадобится
отмена — AbortController, не ref-флаг. Урок процесса: review-fix
коммиты, трогающие UI-логику, требуют такой же playwright-проверки,
как и фичевые (баг жил с d3a9ad5, потому что после fix-коммита UI не
перегонялся). (Сессия 60.)

## StrictMode + «скип первого прогона» ref = deep-link сброшен на mount

**Симптом:** прямой переход на `/hadith/hadiths?page=3` (или шара ссылки,
F5) грузил стр.1, а не 3 — `?page=` молча исчезал из URL. Юнит-тест
«init из ?page=3» при этом ЗЕЛЁНЫЙ (ложное успокоение).

**Причина:** в `usePagedList` (C20) сброс-на-стр.1 при смене query/фильтров
пропускал mount через `firstQueryRunRef = useRef(true)` (первый прогон
ставит false и return). Но под `<StrictMode>` dev двоит mount-эффекты
(setup → cleanup → setup): 1-й прогон ставит ref в false, 2-й видит false
→ трактует mount как «реальную смену query» → сброс на стр.1 + strip
`?page=`. Тот же класс, что unmounted-ref гард выше: ref-флаг «первого
раза» неидемпотентен к double-invoke. Юнит не поймал — RTL `render` НЕ
оборачивает в StrictMode (а dev-сервер — да).

**Решение:** сброс — по сравнению со СНИМКОМ предыдущих query/deps
(`prevQueryDepsRef`, `!Object.is`), а не «первый прогон». Первый прогон
лишь запоминает значения (без сброса); StrictMode-повтор видит те же →
no-op; сброс только при реальной смене. Регресс-тест — `renderHook` со
StrictMode-wrapper'ом (`wrapperWith(..., strict=true)`), ключевой ассерт
«бэк page=0 НЕ запрашивался». Урок: deep-link / URL-state UX-фичи
проверять playwright'ом против ЖИВОГО (StrictMode) сервера — юнит на RTL
это пропускает. (Сессия 62.)
