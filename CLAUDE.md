...
**Install:** the `graphify` CLI is installed globally via `uv tool install graphifyy`
(on PATH at `~/.local/bin/graphify`, shared across projects). Just call `graphify`.
Upgrade with `uv tool upgrade graphifyy`.
