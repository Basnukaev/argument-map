  ...
  return (
    <PanelSection icon={Info} title={t('node.section.metadata')} defaultOpen>
      ...
      <dl className="grid grid-cols-[100px_1fr] gap-x-3 gap-y-2 text-xs">
        <dt className="text-ink-500">{t('node.created_at')}</dt>
        ...
        {wasUpdated && (
          <>
            <dt className="text-ink-500">{t('node.updated_at')}</dt>
            <dd className="text-ink-700">
        ...
        <dt className="text-ink-500">{t('node.author')}</dt>
        <dd className="font-mono text-ink-700" title={node.createdBy}>
          <bdi dir="ltr">{shortId(node.createdBy)}</bdi>
        </dd>

        <dt className="text-ink-500">{t('node.id')}</dt>
        <dd className="font-mono text-ink-700" title={node.id}>
