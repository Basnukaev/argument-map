...
describe('TopicMembersModal', () => {
  ...
  it('POST с UUID и role MEMBER при клике "Добавить"', async () => {
    ...
    server.use(
      ...
      http.post(
        `${BASE}/api/v1/topics/${TOPIC_ID}/members`,
        async ({ request }) => {
  ...
  it('toast error при невалидном UUID, POST не отправляется', async () => {
    server.use(
      ...
      http.post(`${BASE}/api/v1/topics/${TOPIC_ID}/members`, () => {
  ...
  it('DELETE при клике на trash icon после confirm', async () => {
    ...
    server.use(
      ...
      http.delete(
        `${BASE}/api/v1/topics/${TOPIC_ID}/members/${MEMBER_ID}`,
        () => {
          deleted = true;
